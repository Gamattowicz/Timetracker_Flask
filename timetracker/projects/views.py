...
def create_project_view():
    ...
    if request.method == "POST":
        ...
        if Project.query.filter_by(name=name).first():
            flash(f"Project with name {name} already exist!", category="error")
            return redirect(url_for("projects.create_project_view"))
        elif Project.query.filter_by(shortcut=shortcut).first():
            flash(
                f"Project with shortcut {shortcut} already " f"exist!", category="error"
            )
            return redirect(url_for("projects.create_project_view"))
        if request.form.get("start_date"):
            ...
            if end_date < start_date:
                ...
                return redirect(url_for("projects.create_project_view"))
            new_project = Project(
                name=name, shortcut=shortcut, phase=phase, end_date=end_date
            )
...
def list_project_view():
    projects = (
        ...
        .outerjoin(Hour, Project.shortcut == Hour.project_shortcut)
        .group_by(Project.id)
        .all()
    )
    ...
    if request.method == "POST":
        if request.form.get("name") != project.name:
            if Project.query.filter_by(name=request.form.get("name")).first():
                flash(
                    f"Project with name {project.name} already exist!", category="error"
                )
                return redirect(
                    url_for("projects.update_project_view", project_id=project.id)
                )
        if request.form.get("shortcut") != project.shortcut:
            if Project.query.filter_by(shortcut=request.form.get("shortcut")).first():
                flash(
                    f"Project with shortcut {project.shortcut} already " f"exist!",
                    category="error",
                )
                return redirect(
                    url_for("projects.update_project_view", project_id=project.id)
        ...
        if request.form.get("start_date"):
            ...
            flash(f"Invalid date of end project", category="error")
            return redirect(
                url_for("projects.update_project_view", project_id=project_id)
            )
        for hour in hours:
...
def schedule_view():
    ...
    for project in projects_list:
        ...
        ax.broken_barh(
            [(start_date, delta)], (y_tick_value, 9), facecolors=colors.pop()
        )
