...
def create_hour_view():
    ...
    if request.method == "POST":
        ...
        if request.form.get("work_date"):
            ...
            if (work_date_formatted - project_start_date).days < 0 or (
                work_date_formatted - project_end_date
            ).days > 0:
                flash(
                    f"Your working hours date is out of the date range of selected project!",
                    category="error",
                )
            ...
            if hour_num > 24:
                flash(
                    f"Your numbers of hours in {work_date} exceeds 24!",
                    category="error",
                )
                return redirect(url_for("hours.create_hour_view"))
            else:
            ...
            # Check if working hours date is within the project date range
            if (date.today() - project_start_date).days < 0 or (
                date.today() - project_end_date
            ).days > 0:
                flash(
                    f"Your working hours date is out of the date range of selected project!",
                    category="error",
            ...
            if hour_num > 24:
                flash(
                    f"Your numbers of hours in {date.today()} exceeds 24!",
                    category="error",
                )
                return redirect(url_for("hours.create_hour_view"))
            else:
                new_hours = Hour(
                    amount=amount, project_shortcut=project_shortcut, user_id=user_id
                )
...
def update_hour_view(hour_id):
    ...
    if request.method == "POST":
        ...
        if hour_num > 24:
            flash(
                f"Your numbers of hours in {hour.work_date} exceeds 24!",
                category="error",
            )
            return redirect(url_for("hours.update_hour_view", hour_id=hour_id))
        else:
    ...
    return render_template(
        ...
        user=current_user,
        form=form,
        date=datetime.strptime(hour.work_date, "%Y-%m-%d").date(),
    )
