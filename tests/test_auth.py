...
def test_registration(client, app):
    ...
    response = client.post(
        "/sign-up",
        data={"username": "test1", "password": "12345678", "confirm": "12345678"},
    )
    ...
    with app.app_context():
        assert User.query.filter_by(username="test1").first() is not None
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"username": "test", "password": "12345678", "confirm": "12345667"}
        ),  # not match confirm password
        (
            {"username": "test", "password": "1234", "confirm": "1234"}
        ),  # too short password
        (
            {"username": "t", "password": "12345678", "confirm": "12345678"}
        ),  # too short username
    ],
)
